...
class LegalAmazonMap:
    ...
    def buildLegalAmazon():
        ...
        fig = px.choropleth_mapbox(DT_SET, geojson=geo_mun, locations=DT_SET.CodIbge, color_discrete_sequence=["#66c1a4"],
                          ...
                          featureidkey="properties.id", zoom=3.5)

        fig.update_geos(fitbounds="locations", visible=False)
        fig.update_layout(mapbox_style="carto-positron", title="Amazônia Legal: Expansão Territorial", margin={"r":0,"t":50,"l":0,"b":0}, showlegend=False)
    ...
    def buildByMun():
        file = open(MUN_GEO, encoding="utf8")
        geo_mun = json.load(file)
        return LegalAmazonMap.build(geo_mun, DT_SET_BY_MUN_DESMATAMENTO, DT_SET_BY_MUN_DESMATAMENTO.CodIbge, "properties.id", ["CodIbge", "Municipio", "Desmatamento"])
    ...
    def buildByState():
        ...
        return LegalAmazonMap.build(geo_state, DT_SET_BY_STATE_DESMATAMENTO, DT_SET_BY_STATE_DESMATAMENTO.Estado, "properties.sigla", ["Estado", "Desmatamento"])

    @staticmethod
    def build(geo, dt_set, location, key, hover_data):
        
        fig = px.choropleth_mapbox(dt_set, geojson=geo, locations=location, color_continuous_scale=["#2c7fb8", "#7fcdbb", "#edf8b1"], color=dt_set.Desmatamento,
